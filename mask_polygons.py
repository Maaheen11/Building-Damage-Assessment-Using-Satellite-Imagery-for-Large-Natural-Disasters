...
def mask_polygons_together(size, shapes):
    ...
    #defining dict for damage severity 
    damage_severity = {
        'no-damage': 1, 
        'minor-damage': 2, 
        'major-damage': 3, 
    ...
    for u, (poly, subtype) in shapes.items():
    # for u in shapes:
        # poly = shapes[u]
        pixel_value = damage_severity.get(subtype,0)
        
        blank =  np.zeros(size, np.uint8)
        fillPoly(blank, [poly], pixel_value)
        # fillPoly(blank, [poly], (1,1,1))

        mask_img = np.maximum(mask_img,blank)
    #     mask_img += blank
...
def get_feature_info(feature):
    ...
    for feat in feature['features']['xy']:
        ...
        props[feat['properties']['uid']] = (np.array(coords, np.int32),subtype)
